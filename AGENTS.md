...
Canonical repo schema: [SCHEMA.md](./SCHEMA.md).
...
- `wiki/templates/`: page skeletons for source/topic/entity/decision pages.

## Architecture
- `raw/` is the immutable source layer.
- `wiki/` is the LLM-maintained synthesis layer.
- `wiki/sources/` holds one page per source.
- `wiki/topics/` holds durable concepts and cross-source synthesis.
- `wiki/entities/` holds people, orgs, and products.
- `wiki/decisions/` holds decision records and rationale.
- `wiki/index.md` must stay content-oriented and point to the right pages.
- `wiki/log.md` must stay append-only and reflect every meaningful action.
...
- Recommended raw buckets:
  ...
  - `raw/notes/`
  - `raw/assets/`
- Ingested sources are immutable. If a source changes, add a new raw file instead of editing the old one.
- When a source does not fit an existing bucket, choose the smallest descriptive path and keep it stable.
...
6. If the change introduces a durable synthesis or policy, add or update a decision record in `wiki/decisions/`.
...
- Keep source text and synthesis separate.
- Do not overwrite a raw source; add a new file or a new source page instead.
...
5. If the answer needs a new durable connection, prefer a topic page over a large catch-all note.
...
4. Check that every wiki page appears in `wiki/index.md`.
5. Check that every nontrivial wiki page starts with a short purpose line.
...
- Decision pages should also use:
  - `## Decision`
  - `## Rationale`
  - `## Risks / Mitigations`
  - `## Verification`
...
- Prefer relative links inside `wiki/`.
...
- `wiki/templates/` for page skeletons
- `raw/articles/`, `raw/papers/`, `raw/notes/`, `raw/assets/` for source organization
...
## Bootstrapping
- On a fresh repo, seed the wiki with at least one source page, one topic page, one decision record, `wiki/index.md`, and `wiki/log.md`.
- Keep the architecture visible in both the schema and the wiki so future sessions can re-discover it without tribal knowledge.
...
- 아키텍처/운영 규칙 변경은 `TYPE: decision_record`로 남긴다.
